...
int main()
{
	...
	std::uint8_t flags { 0b0100'1101 };
	std::cout << static_cast<std::bitset<8>>(flags) << "\n";
	
	std::cout << "bit 0 is "
		<< (static_cast<bool>(flags & mask0) ? "on\n" : "off\n");

	// to reset a bit (turn on to off, or keep turned off if already off)
	// use the AND & with the NOT ~ bitwise operator
	//
	// 0 0 1 1 flags
	// 0 1 0 1 mask
	// = = = =
	// 0 0 1 0 reset bit
	flags &= static_cast<std::uint8_t>(~mask0);
	...
	// to turn off multiple bits at the same time
	flags &= static_cast<std::uint8_t>(~(mask1 | mask2));
